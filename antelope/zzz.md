...
2. **「A 未覆盖 且 无可选值」的目标列 → 指定值 / 多行值文件，整列连续循环**：
   ...
   - **`file` 优先于 `value`**；`file` 可写绝对路径，或相对 `antelope/values/<类别>/`、
     `antelope/values/`、仓库根、当前工作目录（按此顺序取第一个存在的）；
     文件不存在、读失败、为空 → 退回 `value`；两者都空（`""` = 未配置）→
     该列**保留 `dataTemp` 占位**并在日志里列出（没找到文件时会打印"已找"的路径清单）。
   - **序列填充**（`sequence`，**默认 `false` = 非序列、整列同值**）：
     `true` → 强制递增，`"S-0001"` 变成 `S-0001, S-0002, …, S-3027`
     （整列连续、跨组不重置；位数按种子左侧补零，如 `0001`/`A-1`；值必须以数字结尾，否则报错）；
     `"auto"` → 值以数字结尾才递增；文件模式按行循环，不受 `sequence` 影响。
