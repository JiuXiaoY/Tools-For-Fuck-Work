...
|---|----------------|----------|-----------|-------------------|
| 1 | **13 个未覆盖列**：其中 **4 列有可选值**（17/126 领型、141 袖型、143 闭合），**9 列无可选值**（19/20/40/41-45/46） | 有可选值列 → AI 选值（**不允许残留占位，⑦⑧ 均硬报错**）；无可选值列（目标列）→ `intermediate_tpl/column_defaults.json` 配 `value` 或 `values/<类别>/` 下的多行值文件（整列连续循环），未配置才保留占位 | 有可选值列：跑 ⑦；无可选值列：填 `column_defaults.json`（见 `zzz.md` ⑤） | 运行 `ai_pick_attributes.py` → ⑧⑨ |
...
6. 无可选值列（**目标列**）→ 由 `intermediate_tpl/column_defaults.json` 配 `value`（单值）
   或 `file`（多行值文件，放 `antelope/values/<类别>/`），按**整列连续循环**填入；
   未配置的列才保留 `dataTemp` 占位（⑧ 日志逐列列出）。
7. **硬性约定（2026-09-11 加）**：有可选值的列**不允许残留占位**——⑦ 写回 M 后校验、
   ⑧ 生成 plan 前再守一道，任一违规 → 打印明细并 `exit 2`（⑦ 中止流程、⑧ 不写 plan）。
