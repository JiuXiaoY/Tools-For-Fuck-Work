...
1. 
   ...
   - **副标题**: `Polaire douce, ceinture à cordon et deux poches latérales, coupe droite pour neige et ski, pantalon thermique` (材质/参数 + 场景/功能)
2. 
...
**1. 关键词表优先 + 中文方向锁定 + 语义联想扩词原则（核心）**  
   ...
   - **避免头部用词单一**：对于相同中文描述的商品（多条），考虑使用不同的前置关键词，不要全部都用 `Pantalon Homme` 或 `Jean Homme` 开头。比如前几个写了 `Pantalon d'Hiver Homme`，后面可以写 `Pantalon de Noël Homme`, `Pantalon Cargo Homme`, `Pantalon de Travail Homme`, `Pantalon de Ski Homme` 来扩大搜索覆盖面。
...
**5. Homme 使用规则**  
   - 依据主标题不放人群词的规则，独立的性别词 `Homme` **仅在主标题的前部出现一次**，用于锁定适配人群

**6. 格式与词形规范**  
   - 每个实词首字母大写，虚词如 `et`、`avec`、`pour` 小写。  
   - 空格分隔，可适当使用逗号分组（不超过两个），禁止句号、感叹号、问号等终止标点。  
   - **字符数严格控制**：主标题 **60~75 字符**，副标题 **105~125 字符**。一个字母或空格视为一个字符。  
   - **禁止重复独立词根**：标题中不能出现完全相同的独立词（如用了 `Pantalon` 就不能再出现另一个独立的 `Pantalon`），但允许包含该词根的搭配词组自由出现（如 `Pantalon de ski`、`Pantalon d'hiver`、`Pantalon cargo`），此为更优做法。
   - **禁止改词库词形**：比如：词库中是 `Imperméable` 就不能写成 `Imperméablement`，`Chaud` 不能写成 `Chaude`，保持词库原词形式
...
**7. 其他注意事项 ！！！特别注意！！！严格遵守！！！**
   ...
   - 尽量不要用词库之外的词或生僻词，不要乱组合词，不要乱造生词！！！
   - 善于进行合理的联想，比如（仅作参考，不合适不要套用）：含有抽绳的大概都是 松紧腰设计，虽然没有提到，也可以加 ceinture à cordon (松紧腰), 只是参考，不要只会用这个
   - 我会分批次输入【中文商品描述】，每批次 12 条，会有多批次，假设是 10 次，就会有 120 条，用词要在大局内不那么同质化，需要多元化。同一批中，副标题里同一个词不得使用超过 7 次！ 
   - 同一个词不得出现在不同副标题的同一位置，同时不要所有副标题格式都一样，可以部分带逗号，部分不带，带逗号的分隔方式可以不一样
   - 词库有很多类别词，要全面记忆，通读，在相关类别找不到适配词，就去其他类别找，哪怕这个类别不相关
   - 某一类词不得连续堆砌超过两个，比如场景词等，必须穿插其他品类词或者功能词等其他类词
   - 示例都是非最终版本，仅参考，不要套用

**Workflow:**
1. 查看 dealExcel_refactoring/prompt_fr/needToGenerate，内化 dealExcel_refactoring/prompt_fr/keywords/pants/pants_.md
2. 将中文描述视作品类/风格/场景的大方向，从词库中提取**高涨幅锁定词**及核心卖点。
3. 严格组装：
   - **主标题**：[Marque] + 核心品类词 + 关键规格（60~75 字符）。
   - **副标题**：提炼适配人群(Homme)、材质、功能、场景等核心词条（105~125 字符）。
